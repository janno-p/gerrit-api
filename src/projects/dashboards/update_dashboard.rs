...
pub fn update_dashboard(project_name: String, dashboard_id: String, dashboard_input: DashboardInput) -> UpdateDashboardBuilder {
    UpdateDashboardBuilder { project_name, dashboard_id, dashboard_input }
}

impl UpdateDashboardBuilder {
    pub async fn execute(&self, client: &GerritClient) -> Result<DashboardInfo, GerritError> {
        client.execute(format!("projects/{}/dashboards/{}", self.project_name, self.dashboard_id), &self.dashboard_input).await
    }
}
